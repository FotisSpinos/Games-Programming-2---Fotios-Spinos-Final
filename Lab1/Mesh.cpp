...
void Mesh::initModel(const IndexedModel& model)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, vertexArrayBuffers[NORMAL_VB]); // tell opengl what type of data the buffer is(GL_ARRAY_BUFFER), and pass the data
...
{
	IndexedModel model = OBJModel("..\\res\\Meshes\\" + filename).ToIndexedModel();	//Loads mesh from file
	initModel(model);		//Initializes model
}
...
void Mesh::draw()
{
	glBindVertexArray(vertexArrayObject);		// Binds vertex array to open gl context
	
	glDrawElements(GL_TRIANGLES, drawCount, GL_UNSIGNED_INT, 0);	// Draws Elements
	
	glBindVertexArray(0);						// Unbinds vertx array to port 0
}