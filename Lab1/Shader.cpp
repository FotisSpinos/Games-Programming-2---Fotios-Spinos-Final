...
void Shader::init(const std::string& filename)
{
	...
	shaders[0] = CreateShader(LoadShader("..\\res\\Shaders\\" + filename + ".vert"), GL_VERTEX_SHADER); // create vertex shader
	shaders[1] = CreateShader(LoadShader("..\\res\\Shaders\\" + filename + ".frag"), GL_FRAGMENT_SHADER); // create fragment shader
...
void Shader::Bind()
{
	glUseProgram(program); //installs the program object specified by the program as part of rendering state
}
...
{
	std::ifstream file;	//represents the file
	...
	{
		while (file.good())	//Error checking
		{
...
void Shader::CheckShaderError(GLuint shader, GLuint flag, bool isProgram, const std::string& errorMessage)
{
	...
	if (isProgram)	//Checks if program is true (not empty)
		glGetProgramiv(shader, flag, &success);
	else
		glGetShaderiv(shader, flag, &success);

	if (success == GL_FALSE)	//Checks if program or shader is not empty (equal to 0)
	{
		if (isProgram)
